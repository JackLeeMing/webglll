...
function makeDebugContext(ctx, opt_onErrorFunc) {
  ...
  opt_onErrorFunc =
    opt_onErrorFunc ||
    function(err, functionName1, args) {
      // apparently we can't do args.join(",");
      var argStr = "";
      for (var ii = 0; ii < args.length; ++ii) {
        argStr +=
          (ii == 0 ? "" : ", ") +
          glFunctionArgToString(functionName1, ii, args[ii]);
      }
      log(
        "WebGL error " +
          glEnumToString(err) +
  ...
  function makeErrorWrapper(ctx, functionName2) {
    return function() {
      var result = ctx[functionName2].apply(ctx, arguments);
      var err = ctx.getError();
      if (err != 0) {
        glErrorShadow[err] = true;
        opt_onErrorFunc(err, functionName2, arguments);
      }
      return result;
    };
  }
...
function makeLostContextSimulatingContext(ctx) {
  ...
  function makeLostContextWrapper(ctx, functionName3) {
    var f = ctx[functionName3];
    return function() {
